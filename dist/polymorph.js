(function (global, factory) {
  ...
  function renderAsAlt(ns, formatter) {
      if (isString(ns)) {
      ...
      for (var i = 0; i < ns.length; i++) {
          var n = ns[i];
          result.push([1, formatter(n[0]), formatter(n[1])]);
          var lastResult = void 0;
          for (var f = 2; f < n.length; f += 6) {
              var p0 = formatter(n[f]);
              var p1 = formatter(n[f + 1]);
              ...
              var isPoint = p0 == dx && p2 == dx && p1 == dy && p3 == dy;
              if (!isPoint || lastResult != (lastResult = ('' + p0 + p1 + p2 + p3 + dx + dy))) {
                  result.push([4, p0, p1, p2, p3, dx, dy]);
              }
          }
      }
      return result;
  ...
  function interpolatePath(paths, options) {
      ...
      return function (offset) {
          ...
          var ns = items[flr]((d - flr) / (flr + 1));
          var render = options.renderAsAltLayer ? renderAsAlt : renderPath;
  ...
  function altParse(layer) {
      var id = layer.id, path = layer.path;
      var xValues = [];
      var yValues = [];
      var points = [];
      for (var i = 0, total = path.length; i < total; i++) {
          var command = path[i];
          var isMove = command[0] === 1;
          if (command[0] === 7) {
              break;
          }
          var x1 = command[1];
          var y1 = command[2];
          var x2 = isNaN(command[3]) ? x1 : command[3];
          var y2 = isNaN(command[4]) ? y1 : command[4];
          var x3 = isNaN(command[5]) ? x1 : command[5];
          var y3 = isNaN(command[6]) ? y1 : command[6];
          if (isMove) {
              points.push(x1, y1);
              xValues.push(x1);
              yValues.push(y1);
